...
import {cosmos, ibc, cosmwasm} from 'stargazejs'
import { minAmountWEIRD as minAmountWEIRDprod, minAmountWEIRDtest,
    denomWEIRDinjective, explorerTxInjective,
    stakingRewardsInjectiveContract,
} from '../../config.json'
...
export async function processTxsInjective (decodedTxs: DecodedTX[], queryClient: StargateClient) {
    ...
    for (const tx of decodedTxs) {
        ...
        for (let i = 0; i < tx.msgs.length; i++) {
            ...
            if (msg.typeUrl === '/cosmos.bank.v1beta1.MsgSend') {
                ...
                for (const token of decodedMsg.amount) {
                    if (token.denom === denomWEIRDinjective) {
                        amount += +token.amount/1e6
                    }
                }
                if (amount >= minAmountWEIRD) {
                    ...
                    if (indexedTx.code === 0) {
                        ...
                        if (countMsgs === 0) {
                            ...
                            telegramMsg = fmt(telegramMsg, '🪙  #Injective #Send  📬\n', 
                ...
                if (
                    decodedMsg.token?.denom === denomWEIRDinjective
                ){
                ...
                if (decodedMsg.contract !== stakingRewardsInjectiveContract) continue;
                ...
                const transfer = indexedTx.events.find(evnt => 
                    evnt.type === 'transfer' && evnt.attributes.find(attr => attr.key === 'amount')?.value.includes(denomWEIRDinjective)
                )?.attributes.find(attr => attr.key === 'amount')
                ?.value.replace(denomWEIRDinjective, '')
                ...
                telegramMsg = fmt(telegramMsg, '🪙  #Injective #Claim_staking_rewards  🖼\n', 
                    'Address ', code(sender), senderDaoDaoNick, ' has claimed staking rewards ', 
                    bold(amount.toString() + ' WEIRD\n')
                )
                countMsgs++
            }
        ...
        if (countMsgs > 0) {
            telegramMsg = fmt(telegramMsg, link('TX link', explorerTxInjective + tx.txId))
            if (tx.memo !== '') {
                telegramMsg = fmt(telegramMsg, '\n\n memo: ', tx.memo)
            }

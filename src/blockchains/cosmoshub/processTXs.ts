...
import { minAmountWEIRD as minAmountWEIRDprod, minAmountWEIRDtest,
    denomWEIRDcosmoshub, explorerTxCosmosHub, darkShadowsCosmosDaoRewardsContract // replace
} from '../../config.json'
        ...
        for (let i = 0; i < tx.msgs.length; i++) {
            ...
            if (msg.typeUrl === MsgSend.typeUrl) { // #Send
                ...
                for (const token of decodedMsg.amount) {
                    if (token.denom === denomWEIRDcosmoshub) {
                ...
                if (
                    decodedMsg.token?.denom === denomWEIRDcosmoshub
                ){
                ...
                if (decodedMsg.contract === darkShadowsCosmosDaoRewardsContract) { // #Claim_rewards_Dark_Shadows_DAO
                    // #Claim_rewards_Dark_Shadows_DAO
                    ...
                    const claimAmount = indexedTx.events.find(evnt => 
                        ...
                        evnt.attributes.find(attr => attr.key === 'denom')?.value === denomWEIRDcosmoshub &&
                    ...
                    telegramMsg = fmt(telegramMsg, '🪙  #CosmosHub #Dark_Shadows_DAO #Claim_rewards\n', 
                        'Address ', code(sender), senderDaoDaoNick, ' has claimed Dark Shadows DAO rewards ', 
                        bold(amount.toString() + ' WEIRD\n')
                    )
                    countMsgs++
                }
